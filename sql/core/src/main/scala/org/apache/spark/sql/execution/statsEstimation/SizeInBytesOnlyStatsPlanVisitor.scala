...
object SizeInBytesOnlyStatsPlanVisitor extends SparkPlanVisitor[Statistics] {
  ...
  private def visitUnaryExecNode(p: UnaryExecNode): Statistics = {
    ...
    var sizeInBytes = (p.child.statsPlan.sizeInBytes * outputRowSize) / childRowSize
    if (sizeInBytes == 0) {
  ...
  override def default(p: SparkPlan): Statistics = p match {
    ...
    case _: SparkPlan => Statistics(sizeInBytes = p.children.map(_.statsPlan.sizeInBytes).product)
  }
  ...
  override def visitHashJoin(p: HashJoin): Statistics = {
    p.joinType match {
      case LeftAnti | LeftSemi =>
        ...
        p.left.statsPlan
      case _ =>
        Statistics(sizeInBytes = p.left.statsPlan.sizeInBytes * p.right.statsPlan.sizeInBytes)
    }
  }
